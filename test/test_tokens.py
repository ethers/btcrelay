...
class TestTokens(object):
    ...
    def testInsufficientPayVerifyTx(self):
        ...
        currFee = int( INIT_FEE_VERIFY_TX * ((127*1024-128)/(127*1024.0))**numHeader )
        # print 'currFee: ' + str(currFee)
        assert self.c.getFeeVerifyTx() == currFee
        ethPaid = currFee - 1
        assert 0 == self.c.verifyTx(txHash, txIndex, siblings, txBlockHash, sender=keySender, value=ethPaid)

        ethPaid = currFee
        ethBal = self.s.block.get_balance(addrSender)
        assert 1 == self.c.verifyTx(txHash, txIndex, siblings, txBlockHash, sender=keySender, value=ethPaid)
